...
def load_clones_state():
    ...
    if f.exists():
        if (time.time() - f.stat().st_mtime) > 3600:
            log.warning(f"Clones state stale ({(time.time()-f.stat().st_mtime)/3600:.1f}h) -> WAIT forcé")
...
def load_m4_state():
    ...
    if f.exists():
        if (time.time() - f.stat().st_mtime) > 3600:
            log.warning(f"M4 state stale ({(time.time()-f.stat().st_mtime)/3600:.1f}h) -> inactif")
...
def load_m5_state():
    ...
    if f.exists():
        if (time.time() - f.stat().st_mtime) > 3600:
            log.warning(f"M5 state stale ({(time.time()-f.stat().st_mtime)/3600:.1f}h) -> inactif")
            return {"active": False, "proposed_multiplier": 0.0}
        with open(f, "r", encoding="utf-8") as fp:
            data = json.load(fp)
            return {
